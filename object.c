...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    char *space_pos = memchr(raw, ' ', null_pos - raw);
    if (!space_pos) { free(raw); return -1; }

    size_t type_len = (size_t)(space_pos - (char *)raw);
    if (strncmp((char *)raw, "blob", type_len) == 0 && type_len == 4)
        *type_out = OBJ_BLOB;
    else if (strncmp((char *)raw, "tree", type_len) == 0 && type_len == 4)
        *type_out = OBJ_TREE;
    else if (strncmp((char *)raw, "commit", type_len) == 0 && type_len == 6)
        *type_out = OBJ_COMMIT;
